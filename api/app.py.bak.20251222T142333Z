...
# ---- UI home (authoritative) ----
@app.get("/", response_class=HTMLResponse)
def home(request: Request):
    ua = (request.headers.get("user-agent") or "").lower()
    # iPad/iPhone Safari has been rendering the main UI as blank — serve a safe UI instead.
    if "ipad" in ua or "iphone" in ua or "ipod" in ua:
        resp = HTMLResponse(render_ipad_home())
        resp.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
        resp.headers["Pragma"] = "no-cache"
        return resp

    resp = HTMLResponse(render_home())
    resp.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    resp.headers["Pragma"] = "no-cache"
    return resp
@app.head('/')
...
# ---- BOOT_OK_BANNER_V9: DIAG ENDPOINT ----
@app.get("/api/diag")
def api_diag():
    return {
        "ok": True,
        "marker": "BOOT_OK_BANNER_V9",
        "time_utc": time.time(),
        "app_py_mtime": 1766376304.5513504,
...
@app.get("/obs-test", response_class=HTMLResponse)
def obs_test():
    import time
    return f"""<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>OBS TEST</title></head>
<body style="margin:0;background:#000;color:#00ff66;font:28px ui-monospace,monospace;display:flex;align-items:center;justify-content:center;height:100vh;">
<div>OBS TEST OK • {time.time()}</div>
</body></html>"""


from fastapi.responses import HTMLResponse

@app.get("/obs-lite", response_class=HTMLResponse)
def obs_lite():
    return """<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>OBS LITE</title></head>
<body style="margin:0;background:#111;color:#fff;font:18px system-ui; padding:24px;">
<h1 style="margin:0 0 10px; color:#a3ff12;">OBS LITE OK</h1>
<p>If this works but <code>/</code> looks blank, the issue is in the main UI (likely the video preview / layout / sizing).</p>
<p>Open: <code>/</code> and <code>/obs-lite</code> and compare.</p>
</body></html>"""
